...
class AlertNotification extends Notification {
  ...
  setChannel(channel: NotificationChannel): void {
    console.log(`Cambiando canal de alerta a: ${channel.constructor.name}`);
    // this.setChannel(channel);
    this.channel = channel;

    // this.chanel vs this.setChannel(channel) -> Ambos funcionan, pero el primero es más directo y evita la sobrecarga de llamar a un método adicional. En este caso, como no hay lógica adicional en el método setChannel, asignar directamente a this.channel es más eficiente y claro.
  }
}
...
class ReminderNotification extends Notification {
  ...
  setChannel(channel: NotificationChannel): void {
    console.log(
      `Cambiando canal de recordatorio a: ${channel.constructor.name}`,
    );
    this.channel = channel;
  }
}
...
class PushNotification extends Notification {
  ...
  override setChannel(channel: NotificationChannel): void {
    console.log(`Cambiando canal de push a: ${channel.constructor.name}`);
    this.channel = channel;
  }
}
