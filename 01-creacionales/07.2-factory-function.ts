...
function createLogger(level: LogLevel) {
  return function (message: string) {
    const outputs: Record<LogLevel, { text: string; color: string }> = {
      info: {
        text: `%c[INFO:${formatDate(new Date())}] ${message}`,
        color: COLORS.black,
      },
      warn: {
        text: `%c[WARNING:${formatDate(new Date())}] ${message}`,
        color: COLORS.blue,
      },
      error: {
        text: `%c[ERROR:${formatDate(new Date())}] ${message}`,
        color: COLORS.purple,
      },
    };

    const output = outputs[level];
    console.log(output.text, output.color);
  };
}
