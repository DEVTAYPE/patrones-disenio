...
class QueryBuilder {
  ...
  execute(): string {
    ...
    this.fields.length
      ? (arrQuery[0] = `SELECT ${this.fields.join(", ")} FROM ${this.table}`)
      : (arrQuery[0] = "SELECT * FROM ${this.table}");

    arrQuery[1] = this.conditions.length
      ? `where ${this.conditions.join(" & ")}`
      : "";
    arrQuery[2] = this.orderFields.length
      ? ` order by ${this.orderFields.join(" ")}`
      : "";

    arrQuery[3] = this.limitCount ? `limit ${this.limitCount}` : "";
