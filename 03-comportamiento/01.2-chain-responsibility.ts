...
class Supervisor extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 1000) {
      console.log(`Supervisor: Approved request of $${amount}.`);
      return;
    }

    console.log(
      `Supervisor: Can't approve request of $${amount}. Passing to the next approver.`,
    );
    this.next(amount);
  }
}
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    ...
    console.log(
      `Manager: Can't approve request of $${amount}. Passing to the next approver.`,
    );
    this.next(amount);
  }
}
...
class Director extends BaseApprover {
  approveRequest(amount: number): void {
    ...
    console.log(
      `Director: Can't approve request of $${amount}. No more approvers available.`,
    );
  }
}
